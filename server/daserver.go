...
type DAServer struct {
	//nolint:all
	log            log.Logger // ignore logger smh
	endpoint       string
	store          KVStore
...
func NewDAServer(host string, port int, store KVStore, useGenericComm bool) *DAServer {
...
func (d *DAServer) HandlePut(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path == "/put" || r.URL.Path == "/put/" { // without commitment
		...
		if d.useGenericComm {
			...
			comm = plasma.NewKeccak256Commitment(input).Encode()
		}
